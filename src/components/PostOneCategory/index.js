...
const PostOneCategory = (props) => {
    const checkUrlPage = props.datapage;
    const inforwebsite = getWebsite();
    var bigPost = props.data.slice(0, 1);
    var remainingPost = props.data.slice(1, 4);
    return (
        <div>
            {
                bigPost.map((item, index) => {
                    ...
                    return (
                        <Link href={item.webUrl.replace(`https://docbao24h.me/${checkUrlPage}`, '')} key={index}>
                            <div className={Styles.firstPost}>
                                <div className={Styles.ImageTitle}>
                                    <img src={item.thumb} alt={item.title} />
                                </div>
                                <a href={item.webUrl.replace(`https://docbao24h.me/${checkUrlPage}`, '')} className={Styles.titlePost}>{item.title}</a>
                            ...
                            <Link href={item.webUrl.replace(`https://docbao24h.me/${checkUrlPage}`, '')} key={index}>
                                <div className={Styles.groupItemMini}>
                                    ...
                                    <div className={Styles.BoxMini}>
                                        <img src={itemFind === undefined ? 'https://ecdn.docbao24h.me/icon_wb/news/docbao24h.png' : itemFind.icon_url_horizontal} style={{ maxWidth: "82px", height: "24px", objectFit: "contain", marginTop: "3px"}} />
