...
import { icon_choban } from "../../../public/resource";
import { getVideoHome } from "../../services/page_video";
...
export default function Video(props) {
    ...
    const ListParams = ListMenuVideo.videos;
    ...
    const fetchMoreData = async () => {
        setLoading(true);
        const res = await fetch(constants.API_BASE_URL + listUrl.CATEGORY_VIDEOS + `_start=${post.length}&` + "db24h=" + constants.API_DB_KEY + `&last=${state.lastTime}&length=10&topic=666666`);
...
export async function getServerSideProps() {
    const ListDataVideo = await getVideoHome();
    return {
        props: {
            ListDataVideo
        }
